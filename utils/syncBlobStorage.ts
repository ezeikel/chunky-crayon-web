...
const syncBlobStorage = async () => {
  ...
  // delete all blobs in dev
  console.log('Deleting all blobs from dev...');
  await Promise.all(
    devBlobs.blobs.map(async (blob) => {
      await del(blob.url, { token: devToken });
    }),
  );

  // copy all prod blobs to dev
  console.log('Copying all blobs from prod to dev...');
  await Promise.all(
    prodBlobs.blobs
      .filter((blob) => !blob.url.endsWith('/')) // Skip folders
      .map(async (blob) => {
        const devKey = blob.pathname;
        console.log(`Uploading ${devKey} to dev...`);

        try {
          const blobBuffer = await fetchWithRetry(blob.url);
          await put(devKey, Buffer.from(blobBuffer), {
            token: devToken,
            access: 'public',
            allowOverwrite: true,
          });
        } catch (error) {
          console.error(`Failed to sync blob ${devKey}:`, error);
          throw error;
        }
      }),
  );
